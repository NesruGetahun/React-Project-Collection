...
function Edit() {
  ...
  const submitHandler = (e) => {
    ...
    } else if (summery.trim() === "" || summery === undefined) {
      return input_dispatch({
        type: "SUMMERY_ERROR",
        payload: "Summery is required!",
      });
    } else if (summery.length > 300) {
      return input_dispatch({
        type: "SUMMERY_ERROR",
  ...
  const editHandler = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("You aren't authorized for this action, sign-up!");
      }
    ...
    <div className="create">
      <form onSubmit={submitHandler}>
        ...
        <div className="action">
          <button onClick={(e) => editHandler()}>Confrim</button>
          <button onClick={(e) => setRedirect(true)}>Cancel</button>
        </div>
      </form>
    </div>
  );
